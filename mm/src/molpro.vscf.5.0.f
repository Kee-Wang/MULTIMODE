...
C**************************************************************
C**************************************************************
      SUBROUTINE MOLVP4(XQ1,XQ2,XQ3,XQ4,MM1,MM2,MM3,MM4,NMODE,
     1NATOM,QQ,XX,XL,XM,RR,MODE1,MODE2,MODE3,MODE4,ENERGY,XK,
     2MOLINC,XTANPM,EVAL,SOL,WRK,MDIM,D,MODINT,MHH1,MHH2,MHH3,MHH4,
     3IND,NP4,NTOT4,MAXPOW,INDFIT)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      CHARACTER*2 SYMBOL(100),CHSYM(4),SYMBAD
      DIMENSION XQ1(MM1),XQ2(MM2),XQ3(MM3),XQ4(MM4),QQ(NMODE)
      DIMENSION ENERGY(MHH4,MHH3,MHH2,MHH1)
      DIMENSION XK(MDIM,MDIM),EVAL(MDIM,3)
      DIMENSION D(MDIM,MDIM),SOL(MDIM,1),WRK(1)
      DIMENSION XTANPM(NMODE),MODINT(NMODE),RR(NATOM,NATOM)
      DIMENSION XX(NATOM,3,1),XL(NATOM,NMODE,3,1),XM(NATOM)
      DIMENSION NP4(NTOT4)
      COMMON/MOLPRO/MOLPRO,MOUT,MINP,SYMBOL,CHSYM,MMLINC
      COMMON/INCREM/INCR
      COMMON/FUNDAM/WAVENM,ATTOJ,BOHR,ELMASS,RAD
      COMMON/WHICH/IWHICH
      COMMON/FITTER/MFIT
...
C**TEMPORARY (DIMENSIONS)
      ...
      COMMON/FILASS/IOUT,INP
108   FORMAT(/,1X,'LEAST SQUARES FITTING OF POTENTIAL FOR MODES: ',
     1I3,2X,I3,2X,I3,2X,I3)
111   FORMAT(1X,'SYMMETRIES ',A2,2X,A2,2X,A2,2X,A2)

      MDT=MODINT(NSMODE)

C**GET MAXPOW AND IENTMX(4) IF RUNNING
      IF(IFITRP.LT.0)THEN
        READ(24)NFIT
        IF(NFIT.GT.IENTMX(4))IENTMX(4)=NFIT
        DO I=1,NFIT
          READ(24)IPOW1,IPOW2,IPOW3,IPOW4
          IF(IPOW1.GT.MAXPOW)MAXPOW=IPOW1
          IF(IPOW2.GT.MAXPOW)MAXPOW=IPOW2
          IF(IPOW3.GT.MAXPOW)MAXPOW=IPOW3
          IF(IPOW4.GT.MAXPOW)MAXPOW=IPOW4
        END DO
        DO MTAU=1,MMTAU/MDT
C**DUMMY READ
          READ(24)(SOL(I,1),I=1,NFIT)
        END DO
        RETURN
      END IF
...
C**SPECIAL CASE B2 WITH B2 WITH B2 WITH B2 WITH B2, ETC.
      ...
      IF(MSYM1.NE.1.AND.(MSYM1.EQ.MSYM3))ISKIP1=0
      IF(MSYM1.NE.1.AND.(MSYM1.EQ.MSYM4))ISKIP1=0
      IF(MSYM1.NE.1.AND.(MSYM1.EQ.MSYMT))ISKIP1=0
      IF(MSYM2.NE.1.AND.(MSYM2.EQ.MSYM3))ISKIP2=0
      IF(MSYM2.NE.1.AND.(MSYM2.EQ.MSYM4))ISKIP2=0
      IF(MSYM2.NE.1.AND.(MSYM2.EQ.MSYMT))ISKIP2=0
      IF(MSYM3.NE.1.AND.(MSYM3.EQ.MSYM4))ISKIP3=0
      IF(MSYM3.NE.1.AND.(MSYM3.EQ.MSYMT))ISKIP3=0
      IF(MSYM4.NE.1.AND.(MSYM4.EQ.MSYMT))ISKIP4=0

      N12=ISYMP(MSYM1,MSYM2)
      N13=ISYMP(MSYM1,MSYM3)
      N14=ISYMP(MSYM1,MSYM4)
      N1T=ISYMP(MSYM1,MSYMT)
      N23=ISYMP(MSYM2,MSYM3)
      N24=ISYMP(MSYM2,MSYM4)
      N2T=ISYMP(MSYM2,MSYMT)
      N34=ISYMP(MSYM3,MSYM4)
      N3T=ISYMP(MSYM3,MSYMT)
      N4T=ISYMP(MSYM4,MSYMT)
...
C**SPECIAL CASE S2*S5 = S1
      IF(N2T.EQ.MSYM1)ISKIP1=0
C**SPECIAL CASE S3*S4 = S1
      IF(N34.EQ.MSYM1)ISKIP1=0
C**SPECIAL CASE S3*S5 = S1
      IF(N3T.EQ.MSYM1)ISKIP1=0
C**SPECIAL CASE S4*S5 = S1
      IF(N4T.EQ.MSYM1)ISKIP1=0
C**SPECIAL CASE S3*S4 = S2
      IF(N34.EQ.MSYM2)ISKIP2=0
C**SPECIAL CASE S3*S5 = S2
      IF(N3T.EQ.MSYM2)ISKIP2=0
C**SPECIAL CASE S4*S5 = S2
      IF(N4T.EQ.MSYM2)ISKIP2=0
C**SPECIAL CASE S4*S5 = S3
      IF(N4T.EQ.MSYM3)ISKIP3=0

      N234=ISYMP(MSYM2,N34)
      N23T=ISYMP(MSYM2,N3T)
      N24T=ISYMP(MSYM2,N4T)
      N34T=ISYMP(MSYM3,N4T)

C**SPECIAL CASE S2*S3*S4 = S1
      IF(N234.EQ.MSYM1)ISKIP1=0
C**SPECIAL CASE S2*S3*S5 = S1
      IF(N23T.EQ.MSYM1)ISKIP1=0
C**SPECIAL CASE S2*S4*S5 = S1
      IF(N24T.EQ.MSYM1)ISKIP1=0
C**SPECIAL CASE S3*S4*S5 = S1
      IF(N34T.EQ.MSYM1)ISKIP1=0
C**SPECIAL CASE S3*S4*S5 = S2
      IF(N34T.EQ.MSYM2)ISKIP2=0

      N234T=ISYMP(N23,N4T)

C**SPECIAL CASE S2*S3*S4*S5 = S1
      IF(N234T.EQ.MSYM1)ISKIP1=0
...
C**SPECIAL CASE B2 WITH B2 WITH B2 WITH B2 WITH B2, ETC.
      ...
      IF(MSYM1.NE.1.AND.(MSYM1.EQ.MSYMT))MN1=1
      IF(MSYM2.NE.1.AND.(MSYM2.EQ.MSYM3))MN2=1
      IF(MSYM2.NE.1.AND.(MSYM2.EQ.MSYM4))MN2=1
      IF(MSYM2.NE.1.AND.(MSYM2.EQ.MSYMT))MN2=1
      IF(MSYM3.NE.1.AND.(MSYM3.EQ.MSYM4))MN3=1
      IF(MSYM3.NE.1.AND.(MSYM3.EQ.MSYMT))MN3=1
      IF(MSYM4.NE.1.AND.(MSYM4.EQ.MSYMT))MN4=1
...
C**SPECIAL CASE S2*S5 = S1
      IF(N2T.EQ.MSYM1)MN1=1
C**SPECIAL CASE S3*S4 = S1
      IF(N34.EQ.MSYM1)MN1=1
C**SPECIAL CASE S3*S5 = S1
      IF(N3T.EQ.MSYM1)MN1=1
C**SPECIAL CASE S4*S5 = S1
      IF(N4T.EQ.MSYM1)MN1=1
C**SPECIAL CASE S3*S4 = S2
      IF(N34.EQ.MSYM2)MN2=1
C**SPECIAL CASE S3*S5 = S2
      IF(N3T.EQ.MSYM2)MN2=1
C**SPECIAL CASE S4*S5 = S2
      IF(N4T.EQ.MSYM2)MN2=1
C**SPECIAL CASE S4*S5 = S3
      IF(N4T.EQ.MSYM3)MN3=1

C**SPECIAL CASE S2*S3*S4 = S1
      IF(N234.EQ.MSYM1)MN1=1
C**SPECIAL CASE S2*S3*S5 = S1
      IF(N23T.EQ.MSYM1)MN1=1
C**SPECIAL CASE S2*S4*S5 = S1
      IF(N24T.EQ.MSYM1)MN1=1
C**SPECIAL CASE S3*S4*S5 = S1
      IF(N34T.EQ.MSYM1)MN1=1
C**SPECIAL CASE S3*S4*S5 = S2
      IF(N34T.EQ.MSYM2)MN2=1

c**SPECIAL CASE S2*S3*S4*S5 = S1
      IF(N234T.EQ.MSYM1)MN1=1
...
C**EVEN NUMBER OF POINTS
      ...
      MX3=MM3/MOLINC
      MX4=MM4/MOLINC
      MDD1=MX1+MOD(MX1,MOLINC)
      MDD2=MX2+MOD(MX2,MOLINC)
      MDD3=MX3+MOD(MX3,MOLINC)
      MDD4=MX4+MOD(MX4,MOLINC)

      IF(IND.EQ.0)THEN
C**CALCULATE MFIT FOR THESE 4 MODES
C**********************************
        MHH1=0
C************************************************INCREMENT
        INCR=MOLINC
...
C************************************************INCREMENT
        INC1=1
        IF(MOD(MID1,2).EQ.0)INC1=MOLINC
        DO M1=1,MMM1
          INCR1=INCR1+1
          IF(INCR1.EQ.INC1)THEN
C**NEED THIS POINT - RESET COUNT
...
C************************************************INCREMENT
          INC2=1
          IF(MOD(MID2,2).EQ.0)INC2=MOLINC
          DO M2=1,MMM2
            INCR2=INCR2+1
            IF(INCR2.EQ.INC2)THEN
              INCR2=0
...
C************************************************INCREMENT
            INC3=1
            IF(MOD(MID3,2).EQ.0)INC3=MOLINC
            DO M3=1,MMM3
              INCR3=INCR3+1
              IF(INCR3.EQ.INC3)THEN
                INCR3=0
...
C************************************************INCREMENT
              INC4=1
              IF(MOD(MID4,2).EQ.0)INC4=MOLINC
              DO M4=1,MMM4
                INCR4=INCR4+1
                IF(INCR4.EQ.INC4)THEN
                  INCR4=0
...
1000      CONTINUE
        END DO
        MFIT=MHH1*MHH2*MHH3*MHH4
C**CALCULATE NFIT FOR THESE 4 MODES
C**********************************
        IOFF4=0
        IOFF3=0
        IOFF2=0
        NFIT=0
C       DO J1=1,MDD1/MN1-1
        DO J1=1,MDD1/MN1
C**SPECIAL CASE B2 WITH B2
          IF(MSYM1.EQ.MSYM2.AND.MN2.EQ.2)
     1    IOFF2=MOD(J1,2)+1-MN2
...
C**TEMPORARY
      ...
      WRITE(IOUT,*)'MDD1,MDD2,MDD3,MDD4 ',MDD1,MDD2,MDD3,MDD4
      WRITE(IOUT,*)'MN1,MN2,MN3,MN4 ',MN1,MN2,MN3,MN4
      WRITE(IOUT,*)'ISKIP1,ISKIP2,ISKIP3,ISKIP4 ',
     1ISKIP1,ISKIP2,ISKIP3,ISKIP4
      WRITE(IOUT,*)'IOFF1,IOFF2,IOFF3,IOFF4 ',IOFF1,IOFF2,IOFF3,IOFF4
      WRITE(IOUT,*)'MSYM1,MSYM2,MSYM3,MSYM4 ',MSYM1,MSYM2,MSYM3,MSYM4
      WRITE(IOUT,*)'MOLVP4 NFIT = ',NFIT
      CALL FLUSH(IOUT)
C**TEMPORARY
        IF(IFITRP.GT.0)WRITE(24)NFIT
        INDFIT=INDFIT+1
        NP4(INDFIT)=NFIT
        IF(NFIT.GT.IENTMX(4))IENTMX(4)=NFIT
C**CALCULATE TERMS FOR THESE 4 MODES
C***********************************
        IOFF4=0
        IOFF3=0
        IOFF2=0
...
C             DO J4=1,MDD4/MN4-1
     ...
     2          THEN
                  NFIT=NFIT+1
                  IF(MN1*(J1-1).GT.MAXPOW)MAXPOW=MN1*(J1-1)
                  IF(MN2*(J2-1)-IOFF2.GT.MAXPOW)MAXPOW=MN2*(J2-1)-IOFF2
                  IF(MN3*(J3-1)-IOFF3.GT.MAXPOW)MAXPOW=MN3*(J3-1)-IOFF3
                  IF(MN4*(J4-1)-IOFF4.GT.MAXPOW)MAXPOW=MN4*(J4-1)-IOFF4
                  IF(IFITRP.GT.0)
     1            WRITE(24)MN1*(J1-1),MN2*(J2-1)-IOFF2,
     2            MN3*(J3-1)-IOFF3,MN4*(J4-1)-IOFF4
                END IF
              END DO
            END DO
          END DO
...
C********************************************************
C**LOOP ROUND TAU QUADRATURE POINTS (MTAU/MODINT(NSMODE))
C********************************************************
      ITAU=INIT-INCTAU
C**TEMPORARY
      WRITE(IOUT,*)'MOLVP4 MMTAU,MDTAU ',MMTAU,MMTAU/MDT
      CALL FLUSH(IOUT)
C**TEMPORARY
      DO MTAU=1,MMTAU/MDT
        ITAU=ITAU+INCTAU
...
C**FIRST GET CONSTANT AT THIS ITAU
        CALL GETPV0(V0,ITAU,NATOM,XX,XM,RR)
...
C**ONE AFTER MIDDLE ONE NEXT
          IF(M1.EQ.MID1)INC1=1
          MX1=M1
          IF(ISKIP1.NE.0.AND.M1.GT.MID1)GO TO 5000
          MH1=MH1+1
          QQ(MODE1)=XQ1(MX1)
...
C************************************************INCREMENT
          ...
          DO M2=1,MMM2
            ...
            IF(M2.EQ.MID2)INC2=1
            MX2=M2
            IF(ISKIP2.NE.0.AND.M2.GT.MID2)GO TO 6000
            MH2=MH2+1
            QQ(MODE2)=XQ2(MX2)
...
C************************************************INCREMENT
            ...
            DO M3=1,MMM3
              ...
              IF(M3.EQ.MID3)INC3=1
              MX3=M3
              IF(ISKIP3.NE.0.AND.M3.GT.MID3)GO TO 7000
              MH3=MH3+1
              QQ(MODE3)=XQ3(MX3)
...
C************************************************INCREMENT
              ...
              DO M4=1,MMM4
                ...
                IF(M4.EQ.MID4)INC4=1
                MX4=M4
                IF(ISKIP4.NE.0.AND.M4.GT.MID4)GO TO 8000
                MH4=MH4+1
                QQ(MODE4)=XQ4(MX4)

C**FITTING GLOBAL POTENTIAL
CC              IF(IWHICH.GT.0)THEN
C
C**GET POTENTIAL AT M1,M2,M3,M4 FOR THIS ITAU
                  DO I=1,NATOM
                    DO K=1,3
                      XX(I,K,1)=(XX(I,K,ITAU)+XL(I,MODE1,K,ITAU)*
     1                QQ(MODE1))/SQRT(XM(I))
                      XX(I,K,1)=XX(I,K,1)+XL(I,MODE2,K,ITAU)*
     1                QQ(MODE2)/SQRT(XM(I))
                      XX(I,K,1)=XX(I,K,1)+XL(I,MODE3,K,ITAU)*
     1                QQ(MODE3)/SQRT(XM(I))
                      XX(I,K,1)=XX(I,K,1)+XL(I,MODE4,K,ITAU)*
...
C**NOW FIT 4-MODE POTENTIAL FOR THIS TAU POINT - ??? WRITE TO (24) ???
        CALL GETV4(XK,EVAL,SOL,MM1,XQ1,MM2,XQ2,MM3,XQ3,MM4,XQ4,ENERGY,
     1  MODE1,MODE2,MODE3,MODE4,MSYM1,MSYM2,MSYM3,MSYM4,MSYMT,NMODE,QQ,
     2  XTANPM,WRK,MDIM,D,MTAU,ITAU,NATOM,XX,XL,XM,RR,MOLINC,MHH1,MHH2,
     3  MHH3,MHH4)

C***********************************************************
C**END LOOP OVER TAU QUADRATURE POINTS (MMTAU/MODINT(NSMODE)
C***********************************************************
...
C**************************************************************
C**************************************************************
      SUBROUTINE GETV4(XK,EVAL,SOL,MM1,XQ1,MM2,XQ2,MM3,XQ3,MM4,XQ4,XV,
     1MODE1,MODE2,MODE3,MODE4,MSYM1,MSYM2,MSYM3,MSYM4,MSYMT,NMODE,QQ,
     2XTANPM,WRK,MDIM,D,MTAU,ITAU,NATOM,XX,XL,XM,RR,MOLINC,MHH1,MHH2,
     3MHH3,MHH4)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      CHARACTER*2 SYMBOL(100),CHSYM(4)
      DIMENSION XV(MHH4,MHH3,MHH2,MHH1)
      DIMENSION XK(MDIM,MDIM),EVAL(MDIM,3)
      DIMENSION SOL(MDIM,1)
      DIMENSION XQ1(MM1),XQ2(MM2),XQ3(MM3),XQ4(MM4)
      DIMENSION QQ(NMODE),XTANPM(NMODE)
      DIMENSION WRK(1),RR(NATOM,NATOM)
      DIMENSION XX(NATOM,3,1),XL(NATOM,NMODE,3,1),XM(NATOM)
...
110   FORMAT(1X,F12.4,5X,F12.4)
111   FORMAT(1X,'SYMMETRIES ',A2,2X,A2,2X,A2,2X,A2)
112   FORMAT(//,13X,'OBSERVED',7X,'CALCULATED',9X,'TOTAL',/)
113   FORMAT(1X,I5,':',F12.4,5X,F12.4,5X,F12.4)
C***********************************************
...
C**POLYNOMIAL DEPENDS ON SYMMETRY
      ...
      IF(MSYM2.NE.1.AND.MSYM4.NE.1.AND.N24.NE.1)NM=N/4
      IF(MSYM3.NE.1.AND.MSYM4.NE.1.AND.N34.NE.1)NM=N/4
      IF(MSYM1.NE.1.AND.N1T.EQ.1)NM=2*NM
      IF(MSYM2.NE.1.AND.N2T.EQ.1)NM=2*NM
      IF(MSYM3.NE.1.AND.N3T.EQ.1)NM=2*NM
      IF(MSYM4.NE.1.AND.N4T.EQ.1)NM=2*NM
...
C*************************************************************
C**TAKE OUT 1-DIM, 2-DIM, 3-DIM POTENTIALS
              CALL GETPV3(VQ3,NMODE,QQ,ITAU,NATOM,XX,XL,XM,RR)
              EVAL(I,1)=XV(MH4,MH3,MH2,MH1)
              EVAL(I,2)=EVAL(I,1)+VQ3
C**WEIGHT (MINIMUM SCALED TO UNITY)
              VSCAL=WAVENM*EVAL(I,1)+1
              EVAL(I,3)=1/VSCAL
C*************************************************************

              NFIT=0
C             DO J1=1,MMM1/MN1-1
              DO J1=1,MMM1/MN1
C**SPECIAL CASE B2 WITH B2
                IF(MSYM1.EQ.MSYM2.AND.MN2.EQ.2)
     1          IOFF2=MOD(J1,2)+1-MN2
...
C**SPECIAL CASE B2 WITH B2 WITH B2 WITH B2
     ...
     1              MSYM1.EQ.MSYM3.AND.MSYM1.EQ.MSYM4)
     2              IOFF4=MOD(J1+J2+J3,2)-MN4
C                   DO J4=1,MMM4/MN4-1
                    DO J4=1,MMM4/MN4
                      IF(MN1*(J1-1).NE.0.AND.MN2*(J2-1)-IOFF2.NE.0.AND.
     1                MN3*(J3-1)-IOFF3.NE.0.AND.MN4*(J4-1)-IOFF4.NE.0)
     2                THEN
                        NFIT=NFIT+1
              XK(I,NFIT)=(XTANH(XTANPM(MODE1)*XQ1(MX1))**(MN1*(J1-1)))*
     1        (XTANH(XTANPM(MODE2)*XQ2(MX2))**(MN2*(J2-1)-IOFF2))*
     2        (XTANH(XTANPM(MODE3)*XQ3(MX3))**(MN3*(J3-1)-IOFF3))*
     3        (XTANH(XTANPM(MODE4)*XQ4(MX4))**(MN4*(J4-1)-IOFF4))
C**
C             XK(I,NFIT)=(XQ1(MX1)**(MN1*(J1-1)))*
C    1        (XQ2(MX2)**(MN2*(J2-1)-IOFF2))*
C    2        (XQ3(MX3)**(MN3*(J3-1)-IOFF3))*
C    3        (XQ4(MX4)**(MN4*(J4-1)-IOFF4))
C*******************
                      END IF
                    END DO
                  END DO
...
C**TEMPORARY
      IF(MTAU.EQ.1)THEN
        WRITE(IOUT,*)'MMM1,MMM2,MMM3,MMM4 ',MMM1,MMM2,MMM3,MMM4
        WRITE(IOUT,*)'MN1,MN2,MN3,MN4 ',MN1,MN2,MN3,MN4
        WRITE(IOUT,*)'IOFF1,IOFF2,IOFF3,IOFF4 ',IOFF1,IOFF2,IOFF3,IOFF4
        WRITE(IOUT,*)'MSYM1,MSYM2,MSYM3,MSYM4 ',MSYM1,MSYM2,MSYM3,MSYM4
        WRITE(IOUT,*)'GETV4 NFIT = ',NFIT
      END IF
C**TEMPORARY
C\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
      IF(JPRINT.GT.0)THEN
        IF(MTAU.EQ.1)WRITE(IOUT,103),MFIT,NFIT
        WRITE(IOUT,*)
        WRITE(IOUT,*)'MTAU = ',MTAU,' ITAU = ',ITAU
      END IF
C\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
C             DO J4=1,MMM4/MN4-1
     ...
     1          MN3*(J3-1)-IOFF3.NE.0.AND.MN4*(J4-1)-IOFF4.NE.0)THEN
                  NFIT=NFIT+1
                  WRITE(IOUT,101)SOL(NFIT,1),MN1*(J1-1),
     2            MN2*(J2-1)-IOFF2,MN3*(J3-1)-IOFF3,MN4*(J4-1)-IOFF4
                END IF
              END DO
            END DO
          END DO
...
C**SPECIAL CASE B2 WITH B2 WITH B2
                    IF(MSYM1.NE.1.AND.MSYM1.EQ.MSYM2.AND.
     1              MSYM1.EQ.MSYM3)IOFF3=MOD(J1+J2,2)-MN3
C**SPECIAL CASE B2 WITH B2 WITH B2
                    IF(MSYM1.NE.1.AND.MSYM1.EQ.MSYM2.AND.
     1              MSYM1.EQ.MSYM4)IOFF4=MOD(J1+J2,2)-MN4
C                   DO J3=1,MMM3/MN3-1
                    DO J3=1,MMM3/MN3
C**SPECIAL CASE B2 WITH B2
                      IF(MSYM3.EQ.MSYM4.AND.MN4.EQ.2)
...
C                     DO J4=1,MMM4/MN4-1
                      DO J4=1,MMM4/MN4
                        IF(MN1*(J1-1).NE.0.AND.MN2*(J2-1)-IOFF2.NE.0.
     1                  AND.MN3*(J3-1)-IOFF3.NE.0.AND.MN4*(J4-1)-IOFF4.
     2                  NE.0)THEN
                          NFIT=NFIT+1
                          EVAL(I,1)=EVAL(I,1)+SOL(NFIT,1)*
     1             (XTANH(XTANPM(MODE1)*XQ1(MX1))**(MN1*(J1-1)))*
     ...
     4             (XTANH(XTANPM(MODE4)*XQ4(MX4))**(MN4*(J4-1)-IOFF4))
C**
C    1             (XQ1(MX1)**(MN1*(J1-1)))*
C    2             (XQ2(MX2)**(MN2*(J2-1)-IOFF2))*
C    3             (XQ3(MX3)**(MN3*(J3-1)-IOFF3))*
C    4             (XQ4(MX4)**(MN4*(J4-1)-IOFF4))
                        END IF
                      END DO
                    END DO
                  END DO
                END DO
C**ADD IN 1-DIM, 2-DIM, 3-DIM POTENTIALS
                CALL GETPV3(VQ3,NMODE,QQ,ITAU,NATOM,XX,XL,XM,RR)

C\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
                IF(JPRINT.GT.2)
     1          WRITE(IOUT,113)I,WAVENM*(XV(MH4,MH3,MH2,MH1)+VQ3),
     2          WAVENM*EVAL(I,1),WAVENM*XV(MH4,MH3,MH2,MH1)
C\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\


          RMS=RMS+EVAL(I,3)*(XV(MH4,MH3,MH2,MH1)+VQ3-EVAL(I,1))**2
...
9000      CONTINUE
        ...
        RMS=WAVENM*DSQRT(RMS/MFIT)
        WRITE(IOUT,*)'WEIGHTED ROOT MEAN SQUARE (CM-1) = ',RMS
        CALL FLUSH(IOUT)
      END IF
C\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
C****************************************************************
C****************************************************************
      SUBROUTINE GETPV0(V,ITAU,NATOM,XX,XM,RR)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION RR(NATOM,NATOM)
      DIMENSION XX(NATOM,3,1),XM(NATOM)
      COMMON/FUNDAM/WAVENM,ATTOJ,BOHR,ELMASS,RAD
      COMMON/FILASS/IOUT,INP
C**CORRECTION FOR (CONSTANT) + 1-DIM + 2-DIM + 3-DIM POTENTIALS
C
...
C****************************************************************
C****************************************************************
      SUBROUTINE GETPV3(V,NMODE,QQ,ITAU,NATOM,XX,XL,XM,RR)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION QQ(NMODE),RR(NATOM,NATOM)
      DIMENSION XX(NATOM,3,1),XL(NATOM,NMODE,3,1),XM(NATOM)
      COMMON/FUNDAM/WAVENM,ATTOJ,BOHR,ELMASS,RAD
      COMMON/FILASS/IOUT,INP
C**CORRECTION FOR (CONSTANT) + 1-DIM + 2-DIM + 3-DIM POTENTIALS
...
C**NEXT GET V1
          DO I=1,NATOM
            DO J=1,3
              XX(I,J,1)=(XX(I,J,ITAU)+XL(I,K,J,ITAU)*QQ(K))/
     1        SQRT(XM(I))
            END DO
          END DO
...
C**NEXT GET V2
              DO I=1,NATOM
                DO J=1,3
                  XX(I,J,1)=(XX(I,J,ITAU)+XL(I,K,J,ITAU)*
     1            QQ(K))/SQRT(XM(I))
                  XX(I,J,1)=XX(I,J,1)+XL(I,L,J,ITAU)*QQ(L)/
     1            SQRT(XM(I))
                END DO
              END DO
              CALL GETPOT(VDP,NATOM,XX,RR)
...
C**NEXT GET V3
     ...
     1                QQ(K))/SQRT(XM(I))
                      XX(I,J,1)=XX(I,J,1)+XL(I,L,J,ITAU)*QQ(L)/
     1                SQRT(XM(I))
                      XX(I,J,1)=XX(I,J,1)+XL(I,N,J,ITAU)*QQ(N)/
     1                SQRT(XM(I))
                    END DO
                  END DO
                  CALL GETPOT(VDP,NATOM,XX,RR)
