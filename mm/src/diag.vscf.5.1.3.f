...
C*****************************************
      ...
      IF(ICID.NE.0)THEN
C**RO-VIB STARTS SEARCH AT D(00) ALWAYS
        KROT=1
      ELSE
C**K-DIAGONEL DOES EACH D(0K) IN TURN
        KROT=KKROT
      END IF
12    CONTINUE
      KROTT=KROT
16    IF(KROTT.GT.4)THEN
      ...
      IF(.NOT.TRIAT)THEN
        ...
        END IF
C**TEMPORARY
C     WRITE(IOUT,*)NSX,ISYMT,KKAA,KKEL,KSTEP,KROT,NS,KROTT
C**TEMPORARY
      END IF
C**ZERO BLOCK
      IF(NTOT(NSX).EQ.0)GO TO 133
...
C**STORE CI ASSIGNMENTS
        KROT=KEL
C**TEMPORARY
C     WRITE(IOUT,*)'I,KROT,ICYC,NSX,IMAX = ',I,KROT,ICYC,NSX,IMAX
C**TEMPORARY
        IF(LCOUNT.GE.0)THEN
